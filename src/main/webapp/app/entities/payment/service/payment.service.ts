...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'payDate'> & {
  payDate?: string | null;
};
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      payDate: payment.payDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      payDate: restPayment.payDate ? dayjs(restPayment.payDate) : undefined,
    };
  }
