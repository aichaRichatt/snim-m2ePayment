...
public class PaymentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all payments").get("/api/payments").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new payment")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"walletMessage\": \"SAMPLE_TEXT\"" +
                                ", \"sapMessage\": \"SAMPLE_TEXT\"" +
                                ", \"payWallet\": \"SAMPLE_TEXT\"" +
                                ", \"payWalletStatus\": \"SAMPLE_TEXT\"" +
                                ", \"paySapStatus\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created payment").get("${new_payment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created payment").delete("${new_payment_url}").headers(headers_http_authenticated))
                .pause(10)
        );
