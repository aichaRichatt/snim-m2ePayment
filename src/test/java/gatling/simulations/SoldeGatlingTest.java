...
/**
 * Performance test for the Solde entity.
 */
public class SoldeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all soldes").get("/api/soldes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new solde")
                        .post("/api/soldes")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"clientRef\": \"SAMPLE_TEXT\"" +
                                ", \"clientName\": \"SAMPLE_TEXT\"" +
                                ", \"clientFirstname\": \"SAMPLE_TEXT\"" +
                                ", \"amount\": 0" +
                                ", \"updatingDate\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_solde_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created solde").get("${new_solde_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created solde").delete("${new_solde_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Solde entity").exec(scn);
